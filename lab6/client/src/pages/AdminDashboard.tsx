...
import { insertGadgetSchema, insertNewsSchema, insertReleaseSchema, Release } from '@shared/schema';
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit((data) => create(data, { onSuccess }))} className="space-y-4 pt-4">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Оценка (1-10)</FormLabel>
              <FormControl>
                <Input type="number" max={10} min={1} {...field} onChange={(e) => field.onChange(parseInt(e.target.value))} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Полный текст обзора</FormLabel>
              <FormControl>
          ...
          render={({ field }) => (
            <FormItem className="flex flex-row items-center space-x-3 space-y-0 rounded-md border p-4">
              ...
              <div className="space-y-1 leading-none">
                <FormLabel>Выбор редакции (на главную)?</FormLabel>
              </div>
            </FormItem>
          )}
        />
        <Button type="submit" className="w-full" disabled={isPending}>
          {isPending ? <Loader2 className="animate-spin" /> : 'Добавить устройство'}
        </Button>
      </form>
    </Form>
  );
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit((data) => create(data, { onSuccess }))} className="space-y-4 pt-4">
        ...
        />
        <Button type="submit" className="w-full" disabled={isPending}>
          {isPending ? <Loader2 className="animate-spin" /> : 'Опубликовать новость'}
        </Button>
      </form>
    </Form>
  );
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit((data) => create(data, { onSuccess }))} className="space-y-4 pt-4">
        ...
        />
        <Button type="submit" className="w-full" disabled={isPending}>
          {isPending ? <Loader2 className="animate-spin" /> : 'Запланировать релиз'}
        </Button>
      </form>
    </Form>
  );
      ...
      <div className="container px-4 py-8">
        ...
        <Tabs defaultValue="gadgets" className="space-y-6">
          <TabsList className="bg-white border p-1 rounded-xl">
            <TabsTrigger value="gadgets" className="rounded-lg">
              Гаджеты
            </TabsTrigger>
              ...
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Dialog open={openGadget} onOpenChange={setOpenGadget}>
                  <DialogTrigger asChild>
                    <Button>
                      <Plus className="w-4 h-4 mr-2" /> Добавить гаджет
                    </Button>
                  </DialogTrigger>
                  <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
                    <DialogHeader>
                      <DialogTitle>Новый обзор</DialogTitle>
                    </DialogHeader>
                    <GadgetForm
                      onSuccess={() => {
                        setOpenGadget(false);
                        toast({ title: 'Гаджет добавлен' });
                      }}
                    />
                  </DialogContent>
                </Dialog>
                          ...
                          <TableRow key={g.id}>
                            ...
                            <TableCell className="text-right">
                              <Button
                                ...
                                size="icon"
                                onClick={() => {
                                  if (confirm('Удалить этот обзор?')) deleteGadget(g.id);
                                }}
          ...
          <TabsContent value="news">
            <Card className="shadow-sm border-none bg-white">
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  <CardTitle>Архив новостей</CardTitle>
                  <CardDescription>Управление публикациями в блоге</CardDescription>
                </div>
                          ...
                          <TableRow key={n.id}>
                            ...
                            <TableCell className="text-right">
                              <Button
                                ...
                                size="icon"
                                onClick={() => {
                                  if (confirm('Удалить эту новость?')) deleteNews(n.id);
                                }}
                                className="text-destructive hover:text-white hover:bg-destructive"
                              >
                                <Trash2 className="w-4 h-4" />
          ...
          <TabsContent value="releases">
            <Card className="shadow-sm border-none bg-white">
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  <CardTitle>План релизов</CardTitle>
                  <CardDescription>Продукты, ожидаемые в ближайшее время</CardDescription>
                </div>
                <Dialog open={openRelease} onOpenChange={setOpenRelease}>
                  <DialogTrigger asChild>
                    <Button>
                      <Plus className="w-4 h-4 mr-2" /> Добавить релиз
                    </Button>
                  </DialogTrigger>
                  <DialogContent className="sm:max-w-xl">
                    <DialogHeader>
                      <DialogTitle>Планирование релиза</DialogTitle>
                    </DialogHeader>
                    <ReleaseForm
                      onSuccess={() => {
                          ...
                          <TableRow key={r.id}>
                            ...
                            <TableCell className="text-right">
                              <Button
                                ...
                                size="icon"
                                onClick={() => {
                                  if (confirm('Удалить из календаря?')) deleteRelease(r.id);
                                }}
                                className="text-destructive hover:text-white hover:bg-destructive"
                              >
                                <Trash2 className="w-4 h-4" />
