...
export default function Home() {
  ...
  const selectedReleases = releases?.filter((r: Release) => date && isSameDay(new Date(r.releaseDate), date)) || [];
      ...
      <section className="py-12 md:py-20 border-b bg-slate-50">
        <div className="container px-4 md:px-6">
          ...
          {loadingGadgets ? (
            ...
            <Carousel className="w-full max-w-6xl mx-auto" plugins={[Autoplay({ delay: 4000 })]} opts={{ align: 'start', loop: true }}>
          ...
          <div className="grid lg:grid-cols-3 gap-12">
              ...
              <div className="flex items-center justify-between">
                ...
                <Link href="/news">
                  <Button variant="link" className="text-primary p-0 h-auto">
                    Все новости <ArrowRight className="ml-2 w-4 h-4" />
                  </Button>
                </Link>
              </div>
            ...
            <div className="lg:col-span-1 space-y-8">
              ...
              <Card className="shadow-md border">
                <CardContent className="p-4">
                  <Calendar
                    ...
                    modifiers={{
                      booked: (date) => releases?.some((r: Release) => isSameDay(new Date(r.releaseDate), date)) || false,
                    }}
                    modifiersStyles={{
              ...
              <div className="space-y-4">
                <h3 className="font-bold text-lg">{date ? format(date, 'd MMMM', { locale: ru }) : 'Выберите дату'}</h3>
                {selectedReleases.length > 0 ? (
                  selectedReleases.map((release: Release) => (
